...
export default function FluxChart({
  ...
  const chartData = prepareData(data);
    let selectedPeakPoint = null;

  if (selectedEvent?.peak_time && chartData.length) {
    const peakTime = new Date(selectedEvent.peak_time).getTime();

    if (!Number.isNaN(peakTime)) {
      selectedPeakPoint = chartData.reduce((closest, point) => {
        ...
        return Math.abs(pointTime - peakTime) <
          Math.abs(closestTime - peakTime)
          ? point
          : closest;
      }, null);
    }
      ...
      <ResponsiveContainer width="100%" height={360}>
        ...
        >
          ...
          />
                    {selectedPeakPoint && (
            <ReferenceLine
              x={selectedPeakPoint.displayTime}
              stroke="#facc15"
              strokeWidth={2}
              strokeDasharray="6 4"
              label={{
                value: "Selected Flare Peak",
                position: "insideTop",
                fill: "#facc15",
                fontSize: 12,
              }}
